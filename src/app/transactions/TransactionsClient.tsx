...
type ApiOk = {
  ...
  facets: {
    seasons: FacetOption[];
    types: FacetOption[];
    teams: FacetOption[]; // rosterId -> owner label
    teamsSeason: number;
  };
};
...
export default function TransactionsClient({ leagueId }: Props) {
  ...
  // Helpful “chips” state
  const hasAnyFilter =
    seasonSel.size > 0 || typeSel.size > 0 || teamSel.size > 0 || !!playerId;
      ...
      <div className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm space-y-4">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(facets?.seasons ?? []).map((s) => {
                const active = seasonSel.has(s.value);
                return (
                  <button
                    key={s.value}
                    className={cn(
                    ...
                    onClick={() => {
                      setSeasonSel((cur) => toggleSet(cur, s.value));
                      resetToPage1();
                    }}
                  >
                    {s.label}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(facets?.types ?? []).map((t) => {
                const active = typeSel.has(t.value);
                return (
                  <button
                    key={t.value}
                    className={cn(
                    ...
                    onClick={() => {
                      setTypeSel((cur) => toggleSet(cur, t.value));
                      resetToPage1();
                    }}
                  >
                    {t.label}
          ...
          {/* Team owner filter + player search */}
              ...
              <div className="text-xs font-semibold text-zinc-600 mb-2">
                ...
                {facets?.teamsSeason ? (
                  <span className="font-normal text-zinc-500">
                    • roster labels from {facets.teamsSeason}
                  </span>
                ) : null}
              </div>

              <div className="max-h-44 overflow-auto rounded-2xl border border-zinc-200 p-2">
                {(facets?.teams ?? []).map((opt) => {
                  const active = teamSel.has(opt.value);
                  return (
                    <label
                      key={opt.value}
                      className="flex items-center gap-2 px-2 py-1 rounded-xl hover:bg-zinc-50 cursor-pointer"
                    >
                      <input
                        type="checkbox"
                        checked={active}
                        onChange={() => {
                          setTeamSel((cur) => toggleSet(cur, opt.value));
                          resetToPage1();
                        }}
                      />
                      <span className="text-sm text-zinc-800">{opt.label}</span>
                      <span className="ml-auto text-xs text-zinc-400">#{opt.value}</span>
                    </label>
                  );
                })}
                {(facets?.teams ?? []).length === 0 && (
                  <div className="text-sm text-zinc-500 px-2 py-1">—</div>
                )}
              </div>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="text-xs text-zinc-500">
            Tip: you can combine season + type + team + player filters
          </div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm text-sm text-zinc-600 flex flex-wrap gap-3 items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              className={cn(
                "rounded-xl px-3 py-2 font-semibold",
                data.page >= data.totalPages ? "text-zinc-400 cursor-not-allowed" : "text-zinc-900 hover:bg-zinc-100"
              )}
                      ...
                      <div className="space-y-1">
                        {t.received.length > 0 && (
                          <div className="text-emerald-700">
                            Added: {t.received.flatMap((r) => r.items).filter(Boolean).join(", ") || "—"}
                          </div>
                        )}
                        {t.sent.length > 0 && (
                          <div className="text-rose-700">
                            Dropped: {t.sent.flatMap((s) => s.items).filter(Boolean).join(", ") || "—"}
                          </div>
                        )}
